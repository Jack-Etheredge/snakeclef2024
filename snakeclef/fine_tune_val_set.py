...
def train_model(cfg: DictConfig) -> None:
    ...
    if resume_from_checkpoint:
        ...
        if "val_fine_tuned" in checkpoint:
            best_validation_loss = checkpoint['validation_loss']
        else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if valid_epoch_loss < best_validation_loss:
            ...
            checkpoint_model(epoch, model, optimizer, criterion, valid_epoch_loss, model_file_path, add_val_flag=True)
...
def create_train_val_loaders(cfg):
    ...
    # # swap train and val to fine-tune on val
    # indices = np.arange(len(closed_dataset_train.target)).reshape(-1, 1)
    # indices, _ = RandomUnderSampler(random_state=1260).fit_resample(indices, closed_dataset_train.target)
    # closed_dataset_train = torch.utils.data.Subset(closed_dataset_train, indices.squeeze())
    # closed_dataset_train, closed_dataset_val = closed_dataset_val, closed_dataset_train

    # swap train and val to fine-tune on val
    combined_train = torch.utils.data.ConcatDataset([closed_dataset_train, closed_dataset_val])
    combined_val = torch.utils.data.ConcatDataset([closed_dataset_train, closed_dataset_val])
    total_target = np.array(list(closed_dataset_train.target) + list(closed_dataset_val.target))
    train_indices = []
    val_indices = []
    for cls in np.unique(closed_dataset_train.target):
        train_count = np.sum(closed_dataset_train.target == cls)
        val_count = np.sum(closed_dataset_val.target == cls)
        cls_indices = (total_target == cls).nonzero()
        n_total = len(cls_indices)
        n_val = max(1, val_count) # at least one sample per class in val
        val_indices = cls_indices[-n_val:]
        train_indices = cls_indices[:-n_val]

    closed_dataset_train = torch.utils.data.Subset(combined_train, val_indices)
    closed_dataset_val = torch.utils.data.Subset(combined_val, train_indices)
    ...
    if cfg["train"]["use_class_weights_venom_loss"]:
